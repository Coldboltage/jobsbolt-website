...
const DefaultJobPage = ({ initialJob }: { initialJob: Job }) => {
  ...
  useEffect(() => {
    ...
    const fetchJob = async () => {
      ...
      const res = await fetch(`http://${process.env.API_URL}:3000/job/${job.id}`, {
        method: 'GET',
        headers: {
  ...
  const sendUserPitch = async () => {
    const JWT = process.env.NEXT_PUBLIC_API_URL_JWT_TOKEN
    await fetch(`http://${process.env.API_URL}:3000/cover-letter/`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${JWT}`, // JWT as a Bearer Token
  ...
  const applyToJob = async () => {
    const JWT = process.env.NEXT_PUBLIC_API_URL_JWT_TOKEN
    await fetch(`http://${process.env.API_URL}:3000/job/application-state/${job.indeedId}/${true}`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${JWT}`, // JWT as a Bearer Token
