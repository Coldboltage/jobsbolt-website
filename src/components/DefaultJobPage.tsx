...
const DefaultJobPage = ({ initialJob, jwt }: { initialJob: Job; jwt: string }) => {
  ...
  useEffect(() => {
    ...
    const fetchJob = async () => {
      const res = await fetch(`https://${process.env.NEXT_PUBLIC_CLIENT_API_URL}:3000/api/job/${job.id}`, {
        method: 'GET',
        headers: {
  ...
  const sendUserPitch = async () => {
    await fetch(`https://${process.env.NEXT_PUBLIC_CLIENT_API_URL}:3000/api/cover-letter/`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${jwt}`, // JWT as a Bearer Token
  ...
  const applyToJob = async () => {
    await fetch(`https://${process.env.NEXT_PUBLIC_CLIENT_API_URL}:3000/api/job/application-state/${job.indeedId}/${true}`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${jwt}`, // JWT as a Bearer Token
